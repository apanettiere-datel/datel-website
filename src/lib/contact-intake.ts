...
async function sendContactEmail(input: ContactIntakeInput) {
  ...
  const toEmail =
    input.team === 'support'
      ? process.env.CONTACT_SUPPORT_EMAIL || defaultRecipients.support
      : process.env.CONTACT_SALES_EMAIL || defaultRecipients.sales
  const fromEmail = process.env.CONTACT_FROM_EMAIL || 'DATEL Website <onboarding@resend.dev>'
