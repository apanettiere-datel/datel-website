...
import {
  PREMIUM_PARTNER_DOMAIN_RATE,
  PREMIUM_PARTNER_MIN_DOMAINS,
  PREMIUM_PARTNER_USAGE_RATE_PER_1000_CALLS,
  ...
  estimatePremiumPartnerMonthlyCost,
  roundCurrency,
...
function formatInteger(value: number) {
  return Math.floor(clampToNonNegativeNumber(value)).toLocaleString()
}

type ResellerPriceEstimatorSectionProps = {
  embedded?: boolean
}

export function ResellerPriceEstimatorSection({ embedded = false }: ResellerPriceEstimatorSectionProps = {}) {
  ...
  const [includePremiumPartnerProgram, setIncludePremiumPartnerProgram] = useState(true)
  const [premiumPartnerDomains, setPremiumPartnerDomains] = useState(10)
  const [premiumPartnerCallRecords, setPremiumPartnerCallRecords] = useState(100000)
  ...
  const pricing = useMemo(() => {
    const premiumPartner = includePremiumPartnerProgram
      ? estimatePremiumPartnerMonthlyCost({
          domains: premiumPartnerDomains,
          callRecords: premiumPartnerCallRecords,
        })
      : null
    ...
    const premiumPartnerDomainMonthly = premiumPartner?.domainCharge ?? 0
    const premiumPartnerUsageMonthly = premiumPartner?.aggregateUsageCharge ?? 0
    const premiumPartnerMonthly = premiumPartner?.monthlyTotal ?? 0
    ...
    return {
      ...
      sentimentRate,
      premiumPartnerEnabled: includePremiumPartnerProgram,
      premiumPartnerDomains: premiumPartner?.domains ?? 0,
      premiumPartnerCallRecords: premiumPartner?.callRecords ?? 0,
      premiumPartnerMinimumDomainsMet: premiumPartner?.minimumDomainsMet ?? false,
      premiumPartnerDomainMonthly,
      premiumPartnerUsageMonthly,
      premiumPartnerMonthly,
      monthlyTotal,
      annualRunRate,
      monthFourTotal,
    ...
    includePremiumPartnerProgram,
    includeRealTime,
    includeRecording,
    includeSentiment,
    includeStorage,
    includeTranscription,
    premiumPartnerCallRecords,
    premiumPartnerDomains,
    realTimeAgents,
    sentimentAgents,
    storageHours,
    tier,
    ...
    <div className="w-full">
      <div className="rounded-2xl border border-[#d7e4ee] bg-white p-8 shadow-[0_8px_24px_rgb(18_52_88/10%)]">
        ...
        <p className="mt-4 max-w-4xl text-base/7 text-[#4f6781]">
          Estimate monthly partner pricing using the latest DATEL pricing sheets. Enter your reseller usage and toggle add-ons to model
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-[#d7e4ee] bg-white p-6 shadow-[0_8px_24px_rgb(18_52_88/8%)]">
            <h2 className="text-lg font-semibold text-[#123458]">2. Premium Partner Baseline</h2>
            <div className="mt-4 grid gap-3">
              <Toggle
                id="include-premium-partner"
                label="Include Premium Partner Program baseline"
                checked={includePremiumPartnerProgram}
                onChange={setIncludePremiumPartnerProgram}
                helper={`Minimum ${PREMIUM_PARTNER_MIN_DOMAINS} domains, ${formatUsd(PREMIUM_PARTNER_DOMAIN_RATE)}/domain/month, and ${formatUsd(PREMIUM_PARTNER_USAGE_RATE_PER_1000_CALLS)} per 1,000 call records.`}
              />
              {includePremiumPartnerProgram ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  <Input
                    id="premium-partner-domains"
                    label="Participating Domains"
                    value={premiumPartnerDomains}
                    onChange={setPremiumPartnerDomains}
                    min={0}
                    helper={`Program minimum: ${PREMIUM_PARTNER_MIN_DOMAINS} domains`}
                  />
                  <Input
                    id="premium-partner-call-records"
                    label="Monthly Call Records"
                    value={premiumPartnerCallRecords}
                    onChange={setPremiumPartnerCallRecords}
                    min={0}
                    step={100}
                    helper={`${formatUsd(PREMIUM_PARTNER_USAGE_RATE_PER_1000_CALLS)} per 1,000 call records`}
                  />
                </div>
              ) : null}
            </div>
            {includePremiumPartnerProgram ? (
              <div className="mt-4 rounded-lg border border-[#d9ece6] bg-[#f7fcfa] px-4 py-3 text-sm text-[#123458]">
                {!pricing.premiumPartnerMinimumDomainsMet ? (
                  <p className="font-semibold text-[#9a3b00]">
                    Minimum of {PREMIUM_PARTNER_MIN_DOMAINS} domains is required for Premium Partner Program participation.
                  </p>
                ) : null}
                <p className="font-semibold">Premium baseline: {formatUsd(pricing.premiumPartnerMonthly)}/mo</p>
                <p className="mt-1 text-[#4f6781]">
                  Domain baseline: {formatUsd(pricing.premiumPartnerDomainMonthly)}
                  {' · '}
                  Aggregate usage: {formatUsd(pricing.premiumPartnerUsageMonthly)}
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div className="rounded-2xl border border-[#d7e4ee] bg-white p-6 shadow-[0_8px_24px_rgb(18_52_88/8%)]">
            <h2 className="text-lg font-semibold text-[#123458]">3. Usage Inputs</h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <Input
                ...
                min={0}
                step={1}
                helper={`${transcriptionTier.label} at ${formatUsd(transcriptionTier.rate)}/hr`}
              />
              <Input
                id="storage-hours"
          ...
          <div className="rounded-2xl border border-[#d7e4ee] bg-white p-6 shadow-[0_8px_24px_rgb(18_52_88/8%)]">
            <h2 className="text-lg font-semibold text-[#123458]">4. Add-Ons</h2>
            <div className="mt-4 grid gap-3">
              ...
              <Toggle
                ...
                helper="Usage-sensitive transcription rate is applied automatically."
              />
              ...
              {includeSentiment ? (
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="grid gap-2">
                    ...
                    >
                      <option value="intro">Intro (Months 1-3) · $24/agent</option>
        ...
        <aside className="rounded-2xl border border-[#d7e4ee] bg-white p-6 shadow-[0_8px_24px_rgb(18_52_88/10%)] lg:sticky lg:top-24 lg:h-fit">
          ...
          <dl className="mt-5 space-y-3 text-sm">
            {pricing.premiumPartnerEnabled ? (
              <>
                <div className="flex items-start justify-between gap-4">
                  <dt className="text-[#4f6781]">Premium Partner Domains</dt>
                  <dd className="font-semibold text-[#123458]">{formatUsd(pricing.premiumPartnerDomainMonthly)}</dd>
                </div>
                <div className="flex items-start justify-between gap-4">
                  <dt className="text-[#4f6781]">Premium Partner Usage</dt>
                  <dd className="font-semibold text-[#123458]">
                    {formatUsd(pricing.premiumPartnerUsageMonthly)} ({formatInteger(pricing.premiumPartnerCallRecords)} records)
                  </dd>
                </div>
              </>
            ) : null}
          ...
          <p className="mt-5 text-xs/5 text-[#4f6781]">
            Pricing assumptions are based on DATEL_CloudSWEET_Pricing.pdf and Premium_Partner.pdf. Quotes are required for more than 7,000 users,
            custom redaction, and reseller-specific discounts.
          </p>
        </aside>
      </div>
    </div>
