...
export function ScheduleDemoSection() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const subject = `Demo request - ${preferredDate || 'TBD'} ${preferredTime || ''}`.trim()
    const body = [
      'Cloud SWEET demo request',
      '',
      `Name: ${firstName} ${lastName}`.trim(),
      `Company: ${company || 'Not provided'}`,
      `Email: ${email || 'Not provided'}`,
      `Phone: ${country || 'US'} ${phoneNumber || 'Not provided'}`,
      ...
      '',
      'Requested from website request-demo form.',
    ].join('\n')
      ...
      >
        ...
        {submitted && (
          <MotionReveal className="mx-auto mt-8 max-w-xl rounded-xl border border-[#0e7c86]/25 bg-[#49d6c8]/15 px-4 py-3 text-sm/6 text-[#0b1b2b]" delay={0.08}>
            Your demo request email draft has been prepared. If your email app did not open, send details to
